...
class Branch(models.Model):
    name = models.CharField(max_length=255)
    Region = models.ForeignKey(Region, on_delete=models.SET_NULL, null=True, blank=True)
    team_leader = models.ForeignKey(TeamLeader, on_delete=models.SET_NULL, null=True, blank=True)
...
class Attachment(models.Model):
    ...
    deed = models.ForeignKey('Deeds', on_delete=models.CASCADE, related_name='attachments',blank=True, null=True)
...
class Deeds(models.Model):
    ...
    nssf_no = models.CharField(max_length=255)
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='deeds', default=1)
    reviewer = models.ForeignKey(User, on_delete=models.CASCADE, related_name='reviewed_deeds',blank=True, null=True)
    employer_name = models.CharField(max_length=255)
    From_Date = models.DateField(null=True, blank=True)
    To_Date = models.DateField(null=True, blank=True)
    First_Installment_Date = models.DateField(null=True, blank=True)
    Second_Installment_Date = models.DateField(null=True, blank=True)
    Branch = models.ForeignKey(Branch, on_delete=models.SET_NULL, null=True, blank=True)
    Deed_source = models.CharField(max_length=255, choices=Source)
    team_leader = models.ForeignKey(TeamLeader, on_delete=models.SET_NULL, null=True, blank=True)
    Legacy_status = models.BooleanField()
    Signed_Deed = models.BooleanField()
    date_created = models.DateField(auto_now_add=True)
    date_updated = models.DateTimeField(auto_now=True)
    state = FSMField(default='initiated',choices=STATUS)
    remark = models.TextField(blank=True)
    ...
    @fsm_log_by
    @fsm_log_description
    @transition(field=state, source='initiated', target='forwarded')
    def forward(self,user, reviewer):
        #self.remark = remark
        self.reviewer = reviewer
        self.save()
        DeedsLog.objects.create(deed=self, user=user, action='forward')
        #pass
    ...
    def cancel(self,user):
        #self.remark = remark
        self.save()
        DeedsLog.objects.create(deed=self, user=user, action='cancel')
        #pass
    ...
    def reject(self,user,remark):
        self.remark = remark
        self.save()
        DeedsLog.objects.create(deed=self, user=user, action='reject', remark=remark)
        #pass
    ...
    def approve(self,user,remark):
        self.remark = remark
        self.save()
        DeedsLog.objects.create(deed=self, user=user, action='approve', remark=remark)
        #pass

class DeedsLog(models.Model):
    # Define your model fields here
    deed = models.ForeignKey(Deeds, on_delete=models.CASCADE, related_name='logs')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='logs')  
    remark = models.TextField()
    action = models.CharField(max_length=255)
    created_at = models.DateTimeField(auto_now=True,null=True, blank=True)from django.db import models
from django_fsm import transition, FSMIntegerField, FSMField, transition
from django_fsm_log.decorators import fsm_log_by, fsm_log_description
from django.contrib.auth.models import User
from django.core.validators import RegexValidator
